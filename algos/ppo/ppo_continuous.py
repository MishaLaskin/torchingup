...
def train(env_name = "InvertedPendulum-v2",
    ...
    class Actor(nn.Module):
        ...
        def gaussian_policy(self,x):
            mu = self.forward(x)
            log_std = -0.5*tr.ones(mu.shape)
            std = tr.exp(log_std)
            noise = tr.normal(tr.zeros(mu.shape),tr.ones(mu.shape))
            pi = mu + noise * std
            return pi,mu,std

        def log_prob(self,s,a):
            pi,mu,std = self.gaussian_policy(s.float())
            a = a.reshape(mu.shape)
            summand =(a-mu)**2/(std+1e-6)**2 + 2*tr.log(std) + tr.log(2*tensor(np.pi))

            logp = log_likelihood(a,mu,std)

            return logp

        def get_action_ops(self,s):
            s = as_tensor(s).float()
            a,mu,std = self.gaussian_policy(s)

            logp = log_likelihood(a,mu,std)
            v = value_estimate(s).item()
            a = a.item()
            return a,logp.item(),v
