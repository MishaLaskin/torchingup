...
class Logger:
    ...
    def print_epoch(self, epoch, loss1, loss2):
        train_r, _ = self.get_vals()

        print('Epoch {0}  Reward {1} Pi Loss {2:.3f}  V Loss {3:.3f}'.format(epoch,train_r,loss1,loss2 ))
        self.reset()
...
def print_hyperparamters(params,is_discrete=False):
    m = 40
    title = 'VPG with Categorical Policy' if is_discrete else 'VPG with Gaussian Policy'
    print('='*m)
    n = m - len('| '+title)-1
    print('| '+title + n*' '+'|')
    print('='*m)
